...
ADDITIONAL_EMBEDDING_MODELS="${ADDITIONAL_EMBEDDING_MODELS:-}"
...
RAGNET_QDRANT_BASE_URL=$qdrant_base_url
...
ollama_model_installed() {
  ...
  if [[ "$use_container" == "true" ]]; then
    if ! output="$(container_exec ollama ollama list 2>/dev/null)"; then
      return 1
    fi
  else
  ...
  printf '%s\n' "$output" |
    awk -v model="$model" 'NR > 1 {
      name = tolower($1)
      wanted = tolower(model)
      if (name == wanted || name == wanted ":latest") {
        found = 1
      }
    } END { exit found ? 0 : 1 }'
}
  ...
  case "$OLLAMA_MODE" in
    ...
    Docker)
      if port_in_use 11434; then
        ...
        warn "OLLAMA_MODE is Docker, but localhost:11434 is already in use. Setup will use the existing host Ollama service."
        return 1
      fi
      return 0
      ;;
...
use_qdrant_container() {
  if compose_service_running qdrant; then
    success "qdrant already appears to be running on port 6333; setup will reuse/update it."
    return 0
  fi

  if port_in_use 6333; then
    warn "Qdrant port 6333 is already open; setup will use the existing host Qdrant service."
    return 1
  fi

  if port_in_use 6334; then
    fail "Qdrant gRPC port 6334 is already in use, but port 6333 is not available as a host Qdrant HTTP service. Stop the conflicting service or run Qdrant locally on port 6333."
  fi
...
pull_ollama_model() {
  ...
  if [[ "$use_container" == "true" ]]; then
    if ollama_model_installed true "$model"; then
      success "Ollama model '$model' is already available."
      return
    fi
  ...
  if command -v ollama >/dev/null 2>&1; then
    if ollama_model_installed false "$model"; then
      success "Ollama model '$model' is already available."
      return
    fi
...
if [[ "$MODE" == "Docker" ]]; then
  ...
  use_qdrant_container && use_qdrant_container_result=true || use_qdrant_container_result=false
