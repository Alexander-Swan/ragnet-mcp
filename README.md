...
The default setup starts RagNet MCP in Docker, starts Qdrant and Ollama in Docker only when matching host services are not already listening, publishes the local indexer executable, pulls the default Ollama embedding model plus `nomic-embed-text` for compatibility, writes repo-local MCP registration files for Visual Studio and VS Code, registers Codex/Codex CLI when `codex` is available on PATH, and registers Claude Code when `claude` is available on PATH.
