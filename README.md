...
After the first `index_workspace` run, RagNet stores file fingerprints in a Qdrant-backed index-state collection named `{CollectionPrefix}-index-state`. Later indexing runs compare the current file list with that state and only re-analyze/re-embed files that changed. Deleted files are removed from the vector store before the state is saved.

The default fingerprint mode is `RagNet:Indexing:FileFingerprintMode = ContentHash`, which reads each scanned file and stores a SHA256 fingerprint. For very large non-git workspaces where full file hashing is too expensive, set `FileFingerprintMode` to `Metadata` to fingerprint by file size and last-write time. Metadata mode is faster, but it can miss edits when a tool preserves both size and timestamp, so use it only when that tradeoff is acceptable.
