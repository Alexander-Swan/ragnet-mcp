...
After each successful vector upsert batch, RagNet saves an incomplete index checkpoint that includes the completed files and the collection currently being written. If indexing is interrupted, rerun the same index command; RagNet resumes from the checkpoint and skips files already written in completed batches. A forced/full reindex still promotes the replacement collection only after the run completes, so MCP search keeps using the last completed collection during an interrupted run.
