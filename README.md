...
Embedding creation is batched and parallelized during indexing. Configure file batch size with `RagNet:Indexing:MaxFilesPerBatch`, concurrent embedding batch count with `RagNet:Indexing:MaxEmbeddingConcurrency`, and embedding batch size with `RagNet:Indexing:MaxEmbeddingBatchSize`; defaults are `16`, `4`, and `16`. Duplicate chunk content in the same file batch is embedded once by chunk-content hash, then reused for matching chunks. Qdrant vector upserts are also chunked with `RagNet:Qdrant:UpsertBatchSize`, default `256`.
