...
Runs Qdrant, Ollama, and `ragnet-mcp` in Docker, and still publishes the local `ragnet-indexer` executable to `bin`. Use this when you want all services containerized while keeping host-path indexing local.
