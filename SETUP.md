...
Source-control change detection is configured with `RagNet:SourceControl:ChangeDetector`. Supported values are `auto`, `git`, `tfs`, and `none`; the default is `auto`. `auto` chooses Git for Git workspaces, otherwise TFVC/TFS when `tf.exe workfold` can resolve the workspace. TFVC support records latest changesets as `tfvc:<changeset>` and uses TFVC history plus pending status for complete incremental deltas. If a detector cannot prove a complete delta, RagNet falls back to the normal file scan/fingerprint comparison. Use `RagNet:SourceControl:TfCommand` to point at a specific `tf.exe` path.
